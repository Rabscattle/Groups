...
public class GroupCommands extends BaseCommand {
    ...
    public void assign(CommandSender sender, Player player, String group, @Single @Default("0s") String duration) {
        try {
            long expire = DurationUtil.parseDuration(duration);
            this.groupManager.assignGroup(player, group, expire).whenComplete((unused, throwable) -> {
